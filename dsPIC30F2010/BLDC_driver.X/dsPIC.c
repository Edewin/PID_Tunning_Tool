...
#define LF 	0x0A
#define BAUD 19200
#define SPEEDMULT	2343750		// factor used to calculate speed = 60*(FCY/256)
...
static volatile double Kps=1;
static volatile double Kis=0;
...
{
	IFS0bits.ADIF = 0;	
	//DesiredSpeed = ADCBUF0; // now Get desired Speed from UART
...
unsigned char data[100];
	...
	//itoa(k, data, 10); //convert k to string into data;
	sprintf((char*)data,"%d",k);
    
	TXPtr = (unsigned char*)data; 
	SendMsg();
	Flags.CalSpeed = 0;
}
